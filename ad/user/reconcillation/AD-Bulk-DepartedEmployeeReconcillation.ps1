...
#
#---------------------------------------------------------------------
# Purpose: Take CSV file provided by HR, validate if person is still active, deactivate if yes, then alert IT, also check for any assets still assigned to them
#---------------------------------------------------------------------
#
# Changelog
...
Write-Host ""
Write-Host "Employee Departure Reconcillation" -Foregroundcolor White -BackgroundColor Blue
Write-Host ""
...
if ($infoWrittenToCSV -eq $true) {
    ...
    Write-Host ""
    Write-Host "At least one employee in the latest departure file from HR"
    Write-Host "is yet to have their account disabled, due to an missing form"
    Write-Host "likely not yet being received by the IT."
    Write-Host ""
    Write-Host "These accounts have been disabled pending completion of relevant actions."
    Write-Host ""    
    Write-Host "Instructions for relevant actions can be found in the relevant process document on the relevant site."
    Write-Host ""
    ...
    Write-Host "4a) Open a new Help Desk ticket, with the user's Manager as the client, CC'ing HR"
    Write-Host "4b) Send a ticket asking for the missing form and/or Outstanding assets."
    Write-Host ""
