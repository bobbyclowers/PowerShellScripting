$ruleName = "TEST: Disclaimer - Single Name Impersonation Sender Name"
...
Write-Host "Getting rule name"
$rule = Get-TransportRule | Where-Object {$_.Identity -contains $ruleName}

Write-Host "Getting display name for all active mailboxes, and write results to variable $displayNames"
$displayNames = "Michael Harris"

Write-Host "Checking if rule exists"
if (!$rule) {
